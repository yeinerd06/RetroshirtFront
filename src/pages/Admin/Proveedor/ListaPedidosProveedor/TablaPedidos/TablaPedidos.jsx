...
const PedidoRow = ({ pedido, expanded, toggleExpandPedido, modulo }) => (
  <tr className="text-center">
    ...
    <td className="py-2 px-4 border-b">{pedido.total}
      <p> {pedido?.totalDevolucion!==null && "- "}{pedido?.totalDevolucion}</p>
      <p className={pedido?.totalDevolucion!==null && "border-t border-gray-900"}
      >{pedido?.totalDevolucion!==null && pedido?.total - pedido?.totalDevolucion}</p>
    </td>
    <td className="py-2 px-4 border-b text-start">
      {pedido.estadoPedido ? "Completado" : "Pendiente"}
      <p>
        {pedido.devolucion === true &&
          !pedido?.devolucionConfirmada &&
          "Con Devolución"}
      </p>
      <p>{pedido?.devolucionConfirmada === true && "Devolución Confirmada"}</p>
    </td>
    <td className="py-2 px-4 border-b ">
      <IconButton
        className={
      ...
      {pedido.estadoPedido &&
        ...
        !pedido.devolucionConfirmada && (
          <Link
            to={`/${modulo}/proveedor/${pedido.proveedor.id}/pedido/${pedido.id}/confirmar/devolucion`}
          >
            <IconButton
              title="Confirmar Devolución"
              className="ml-3 bg-red-900 hover:bg-red-700 text-xl"
              onClick={() => console.log(pedido)}
            >
              <TbTruckReturn />
            </IconButton>
        ...
        <table className="min-w-full table-auto border border-gray-300">
          <thead className="bg-gray-200">
            <tr>
              ...
              <th className="py-2 px-4 border-b">Devolución Confirmada</th>
            </tr>
          </thead>
          <tbody>
            {productos.map((producto, index) => (
              <tr key={index} className="text-center">
                ...
                <td className="py-2 px-4 border-b text-start text-xs">
                  {producto?.devolucionProveedor !== null && (
                    <>
                      ...
                      <p>
                        {new Date(
                          producto?.devolucionProveedor?.fechaDevolucion
                        ).toLocaleDateString()}
                      </p>
                    </>
                  )}
                ...
                <td className="py-2 px-4 border-b text-start text-xs">
                  {producto?.devolucionProveedor?.estadoDevolucion && (
                    <>
                      <div className="flex justify-between">
                        <p> Reintegro:</p>
                        <p>
                          {producto?.devolucionProveedor?.cantidad -
                            producto?.devolucionProveedor?.cantidadDevolucion}
                        </p>
                      </div>
                      ...
                      <p>
                        {new Date(
                          producto?.devolucionProveedor?.fechaConfirmarDevolucion
                        ).toLocaleDateString()}
                      </p>
                    </>
                  )}
